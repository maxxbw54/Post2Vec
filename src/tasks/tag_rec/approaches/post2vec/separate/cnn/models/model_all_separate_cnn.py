...
def get_topK_predictions_foreach_q(q, K,model, args,tag_vocab):
    model.eval()
    topK_tags = []
    q = [q]
    with torch.no_grad():
        # features
        t = get_specific_comp_list("title", q)
        dt = get_specific_comp_list("desc_text", q)
        dc = get_specific_comp_list("desc_code", q)
        ...
        pred = model(t, dt, dc).cpu().detach().numpy()

        top_idx_list = sorted(range(len(pred[0])), key=lambda i: pred[0][i])[-K:]

        for idx in top_idx_list:
            topK_tags.append(list(tag_vocab.keys())[list(tag_vocab.values()).index(idx)])
